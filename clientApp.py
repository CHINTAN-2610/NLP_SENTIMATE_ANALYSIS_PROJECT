...
from chintan_prediction.predictApp import PredictApi
from chintan_training.trainApp import TrainApi
from chintan_utils.utils import createDirectoryForUser, extractDataFromTrainingIntoDictionary, \
...
def predictRoute():
    ...
    try:
        if request.json['text'] is not None and request.json['userId'] is not None and request.json['projectId'] is not None:
            ...
            stop_word_filepath = "data/stopwords.txt"
            result = PredictApi(stop_word_filepath).executePreocessing(text,jsonFilePath,modelPath,vectorPath)
...
def trainModel():
    ...
    try:
        ...
        modelscore = TrainApi(stop_word_filepath).training_model(jsonpath,modelPath)
